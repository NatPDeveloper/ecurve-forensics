...
const balancesFile = './phase2.json';
...
balancesFileJson.forEach(el => {
    if(el.scope) {
        const USDC = typeof(el.USDC) == "string" ? 0 : Number(el.USDC);
        const DAI = typeof(el.DAI) == "string" ? 0 : Number(el.DAI);
        if(USDC > 0.0001) {
            const temp = JSON.parse(JSON.stringify(template));
            temp.account = tokenContract2;
            temp.data.to = el.scope;
            temp.data.quantity = `${USDC.toFixed(4)} USDT`;
            actions.push(temp);
            console.log(el.USDC)
            console.log(typeof(el.USDC))
        }
        if(DAI > 0.0001) {
            const temp = JSON.parse(JSON.stringify(template));
            temp.account = tokenContract2;
            temp.data.to = el.scope;
            temp.data.quantity = `${DAI.toFixed(4)} USDT`;
            actions.push(temp);
            console.log(el.DAI)
            console.log(typeof(el.DAI))
        }
    }
});
