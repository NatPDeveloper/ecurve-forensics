...
// const balancesFile = './tri_balances.json';
// const balancesFile = './usn_balances.json';
// const balancesFile = './vig_balances.json';
const balancesFile = './eosdt_balances.json';
const rpcUrl = 'https://mainnet.eosn.io';
const tokenContract1 = "dadusdtokens";
const tokenContract2 = "tethertether";
...
balancesFileJson.forEach(el => {
    if(el.scope) {
        const USDC = parseFloat(el.USDC.replace(/,/g, ''));
        const DAI = parseFloat(el.DAI.replace(/,/g, ''));
        const USDT = parseFloat(el.USDT.replace(/,/g, ''));
        if(USDC > 0.1) {
            const temp = JSON.parse(JSON.stringify(template));
            temp.account = tokenContract1;
            temp.data.to = el.scope;
            temp.data.quantity = `${USDC.toFixed(6)} RUSDC`;
            actions.push(temp);
        }
        if(DAI > 0.1) {
            const temp = JSON.parse(JSON.stringify(template));
            temp.account = tokenContract1;
            temp.data.to = el.scope;
            temp.data.quantity = `${DAI.toFixed(6)} RDAI`;
            actions.push(temp);
        }
        if(USDT > 0.1) {
            const temp = JSON.parse(JSON.stringify(template));
            temp.account = tokenContract2;
            temp.data.to = el.scope;
            temp.data.quantity = `${USDT.toFixed(4)} USDT`;
            actions.push(temp);
        }
    }
});

console.log(`${balancesFile} accounts: ${actions.length}`);
...
const propose = async (splitArray, i) => {
    ...
    const proposeInput = {
        ...
        requested,
        trx: {
            expiration: '2022-03-29T23:11:48',
            ref_block_num: 22480,
